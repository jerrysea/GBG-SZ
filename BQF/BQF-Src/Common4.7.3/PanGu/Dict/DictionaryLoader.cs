...
namespace PanGu.Dict
{
    ...
    {
        public static readonly Framework.Lock Lock = new PanGu.Framework.Lock();
                ...
                try
                {
                    if (MainDictChanged())
                    {
                        try
                        {
                            ...
                            Segment.Statics.Instance._WordDictionary.Load(_DictionaryDir + "Dict.dct");
                            _MainDictLastTime = GetLastTime("Dict.dct");
                        }
                        finally
                        {
                    ...
                    if (ChsNameChanged())
                    {
                        try
                        {
                            ...
                            Segment.Statics.Instance._ChsName.LoadChsName(_DictionaryDir);
                            _ChsSingleLastTime = GetLastTime(Dict.ChsName.ChsSingleNameFileName);
                            _ChsName1LastTime = GetLastTime(Dict.ChsName.ChsDoubleName1FileName);
                            _ChsName2LastTime = GetLastTime(Dict.ChsName.ChsDoubleName2FileName);
                        }
                    ...
                    if (StopWordChanged())
                    {
                        try
                        {
                            ...
                            Segment.Statics.Instance._StopWord.LoadStopwordsDict(_DictionaryDir + "Stopword.txt");
                            _StopWordLastTime = GetLastTime("Stopword.txt");
                        }
                        finally
                        {
                    ...
                    if (Segment.Statics.Instance._Synonym.Inited)
                    {
                        if (SynonymChanged())
                        {
                            try
                            {
                                ...
                                Segment.Statics.Instance._Synonym.Load(_DictionaryDir);
                                _SynonymLastTime = GetLastTime(Dict.Synonym.SynonymFileName);
                            }
                            finally
                            {
                    ...
                    if (Segment.Statics.Instance._Wildcard.Inited)
                    {
                        if (WildcardChanged())
                        {
                            try
                            {
                                Segment.Statics.Instance._Wildcard.Load(_DictionaryDir);
                                _WildcardLastTime = GetLastTime(Dict.Wildcard.WildcardFileName);
                            }
                            finally
                            {
