...
namespace PanGu
{
    public class Segment
    {
        ...
        {
            public readonly static Statics Instance = new Statics();

            public bool _Inited = false;
            public Dictionary<string, string> _InfinitiveVerbTable = null;

            public Dict.WordDictionary _WordDictionary = null;
            public Dict.ChsName _ChsName = null;
            public Dict.StopWord _StopWord = null;
            public Dict.Synonym _Synonym = null;
            public Dict.Wildcard _Wildcard = null;

            public Dict.DictionaryLoader _DictLoader;
        }
    ...
    //@"[０-９\d]+|[^ａ-ｚＡ-Ｚa-zA-Z0-9０-９\u4e00-\u9fa5]|[ａ-ｚＡ-Ｚa-zA-Z]+|[\u4e00-\u9fa5]+";
        ...
        private bool MergeEnglishSpecialWord(string orginalText, SuperLinkedList<WordInfo> wordInfoList, ref SuperLinkedListNode<WordInfo> current)
        {
            ...
            if (last >= 0)
            {
                ...
                WordAttribute wa = Statics.Instance._WordDictionary.GetWordAttr(newWord);
        ...
        {
            string stem;
            if (Statics.Instance._InfinitiveVerbTable.TryGetValue(word, out stem))
            {
                return stem;
            }
            ...
            while (cur != null)
            {
                ...
                switch (cur.Value.WordType)
                {
                    case WordType.SimplifiedChinese:
                        ...
                        PanGu.Framework.AppendList<Dict.PositionLength> pls = Statics.Instance._WordDictionary.GetAllMatchs(inputText, _Options.ChineseNameIdentify);
                        PanGu.Match.ChsFullTextMatch chsMatch = new PanGu.Match.ChsFullTextMatch(Statics.Instance._WordDictionary);
                                ...
                                if (Framework.Regex.GetMatchStrings(cur.Value.Word, PATTERNS, true, out output))
                                {
                                    ...
                                    if (outputCount > 1)
                                    {
                                        ...
                                        foreach (string splitWord in output)
                                        {
                                            ...
                                            {
                                                wi = new WordInfo(splitWord, POS.POS_D, 1);
        ...
        private void FilterStopWord(SuperLinkedList<WordInfo> wordInfoList)
        {
            ...
            while (cur != null)
            {
                if (Statics.Instance._StopWord.IsStopWord(cur.Value.Word, 
        ...
        private void ProcessAfterSegment(string orginalText, SuperLinkedList<WordInfo> result)
        {
            ...
            if (_Options.SynonymOutput)
            {
                ...
                while (node != null)
                {
                    List<string> synonyms = Statics.Instance._Synonym.GetSynonyms(node.Value.Word);
            ...
            if (_Options.WildcardOutput)
            {
                ...
                while (node != null)
                {
                    List<Dict.Wildcard.WildcardInfo> wildcards =
                        Statics.Instance._Wildcard.GetWildcards(node.Value.Word);
            ...
            //{
                //CustomRuleManagement mgr = new CustomRuleManagement(orginalText);
                //mgr.ExecuteRules(result);
            //}
        }
        ...
        public ICollection<WordInfo> DoSegment(string text, Match.MatchOptions options, Match.MatchParameter parameters)
        {
            ...
            try
            {
                ...
                if (_Options == null)
                {
                    _Options = Setting.PanGuSettings.Statics.Instance.Config.MatchOptions;
                }

                if (_Parameters == null)
                {
                    _Parameters = Setting.PanGuSettings.Statics.Instance.Config.Parameters;
                }
        ...
        {
            Statics.Instance._WordDictionary = new PanGu.Dict.WordDictionary();
            string dir = Setting.PanGuSettings.Statics.Instance.Config.GetDictionaryPath();
            Statics.Instance._WordDictionary.Load(dir + "Dict.Dct");

            Statics.Instance._ChsName = new PanGu.Dict.ChsName();
            Statics.Instance._ChsName.LoadChsName(Setting.PanGuSettings.Statics.Instance.Config.GetDictionaryPath());


            Statics.Instance._WordDictionary.ChineseName = Statics.Instance._ChsName;

            Statics.Instance._StopWord = new PanGu.Dict.StopWord();
            Statics.Instance._StopWord.LoadStopwordsDict(dir + "Stopword.txt");

            Statics.Instance._Synonym = new PanGu.Dict.Synonym();

            if (Setting.PanGuSettings.Statics.Instance.Config.MatchOptions.SynonymOutput)
            {
                Statics.Instance._Synonym.Load(dir);
            }

            Statics.Instance._Wildcard = new PanGu.Dict.Wildcard(Setting.PanGuSettings.Statics.Instance.Config.MatchOptions,
                Setting.PanGuSettings.Statics.Instance.Config.Parameters);

            if (Setting.PanGuSettings.Statics.Instance.Config.MatchOptions.WildcardOutput)
            {
                Statics.Instance._Wildcard.Load(dir);
            }

            Statics.Instance._DictLoader = new PanGu.Dict.DictionaryLoader(Setting.PanGuSettings.Statics.Instance.Config.GetDictionaryPath());
        }

        private static void InitInfinitiveVerbTable()
        {
            if (Statics.Instance._InfinitiveVerbTable != null)
            {
                return;
            }

            Statics.Instance._InfinitiveVerbTable = new Dictionary<string, string>();
            ...
            using (System.IO.StringReader sr = new System.IO.StringReader(AnalyzerResource.INFINITIVE))
            {
                ...
                while (!string.IsNullOrEmpty(line))
                {
                    ...
                    for (int i = 1; i < 3; i++)
                    {
                        ...
                        if (!Statics.Instance._InfinitiveVerbTable.ContainsKey(key))
                        {
                            Statics.Instance._InfinitiveVerbTable.Add(key, strs[0].Trim().ToLower());
                        }
                    }
