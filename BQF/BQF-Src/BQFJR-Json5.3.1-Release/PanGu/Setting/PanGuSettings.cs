...
namespace PanGu.Setting
{
    ...
    public class PanGuSettings
    {
        public class Statics
        {
            ...
            public PanGuSettings _Config;

            public PanGuSettings Config
            {
                get
                {
                    return _Config;
        ...
        //    try
        //    {
        //        byte[] config = Encoding.UTF8.GetBytes(xml);
        //        MemoryStream ms = new MemoryStream(config);
        //        _Config = XmlSerialization<PanGuSettings>.Deserialize(ms);
        //    }
        //    catch
        //    {
        ...
        static public void Load(string fileName)
        {
            if (System.IO.File.Exists(fileName))
            {
                try
                {
                    ...
                    {
                        Statics.Instance._Config = XmlSerialization<PanGuSettings>.Deserialize(fs);
                    }
                }
                catch
                {
                    Statics.Instance._Config = new PanGuSettings();
                }
            }
            else
            {
        ...
        static public void Save(string fileName)
        {
            ...
            {
                XmlSerialization<PanGuSettings>.Serialize(Statics.Instance.Config, Encoding.UTF8, fs);
            }
        }
        ...
        public string GetDictionaryPath()
        {
            ...
            return @"C:\Dict\";//PanGu.Framework.Path.AppendDivision(path, '\\');
        }
