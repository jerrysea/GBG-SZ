...
namespace PanGu.Framework
{
    ...
    public abstract class DFA <Token, Function>
    {
        public class Statics
        {
            ...
            public DFAState<Token, Function>[] States = new DFAState<Token, Function>[32];
        ...
        protected static readonly object InitLockObj = new object();

        protected static readonly int EofAction = 0;
        ...
        {
            if (state.Id >= Statics.Instance.States.Length)
            {
                int newLength;

                if (state.Id < 2 * Statics.Instance.States.Length)
                {
                    newLength = 2 * Statics.Instance.States.Length;
                }
                else
                {
                ...
                DFAState<Token, Function>[] oldStates = Statics.Instance.States;

                Statics.Instance.States = new DFAState<Token, Function>[newLength];

                oldStates.CopyTo(Statics.Instance.States, 0);
            }

            if (Statics.Instance.States[state.Id] != null)
            {
                throw new DFAException("state.Id must be equal to States index!", 0, state.Id);
            }

            Statics.Instance.States[state.Id] = state;
        ...
        {
            Debug.Assert(Statics.Instance.States != null);

            if (Statics.Instance.States.Length == 0)
            {
            ...
            CurrentState = Statics.Instance.States[CurrentState].NextState(action, this, out isElseAction);
            ...
            if (CurrentState >= Statics.Instance.States.Length)
            {
                throw new DFAException("Bad DFA state!", action, CurrentState);
            }

            if (Statics.Instance.States[CurrentState] == null)
            {
                throw new DFAException("Bad DFA state!", action, CurrentState);
            }

            if (!Statics.Instance.States[CurrentState].NoFunction)
            {
                Statics.Instance.States[CurrentState].DoThings(action, this);
            }

            if (!Statics.Instance.States[CurrentState].IsQuitState && !QuitManually)
            {
                return DFAResult.Continue;
            }
            else
