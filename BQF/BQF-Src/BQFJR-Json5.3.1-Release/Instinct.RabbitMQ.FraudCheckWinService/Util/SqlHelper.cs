...
namespace Instinct.RabbitMQ.FraudCheckWinService.Util
{
    ...
    public sealed partial class SqlHelper
    {
        ...
        /// <param name="connectionString">数据库连接字符串</param>
        /// <param name="table">数据表</param>
        public static void BulkInsert(string connectionString, DataTable table, string desTableName)
        {
            if (string.IsNullOrEmpty(table.TableName)) throw new Exception("DataTable.TableName属性不能为空");
            using (SqlBulkCopy bulk = new SqlBulkCopy(connectionString))
            {
        ...
        public static void BulkInsert(SqlConnection cnn, DataTable table,string desTableName,SqlTransaction tran)
        {
            if (string.IsNullOrEmpty(desTableName)) throw new Exception("DataTable.TableName属性不能为空");
            using (SqlBulkCopy bulk = new SqlBulkCopy(cnn,SqlBulkCopyOptions.Default,tran))
            {
                bulk.BatchSize = BatchSize;
                bulk.BulkCopyTimeout = CommandTimeOut;
                bulk.DestinationTableName = desTableName;
