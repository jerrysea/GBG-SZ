...
namespace SCM.RabbitMQClient
{
    ...
    /// MQ 简单类
    /// </summary>
    public class RabbitMqClientJSONSimple : IRabbitMqClient
    {
        #region 构造
        /// <summary>
        /// 构造函数
        ...
        public RabbitMqClientJSONSimple(string host, string vhost, string queue, int port, string user, string password, string encode, bool needdeclare = true, bool synchronizationFlag = false,bool autoAck=false,ushort heartbeat=60)
        {
            MQHost = host;
            MQVHost = vhost;
            MQPort = port;
        ...
        public void TriggerEventMessage(EventMessage eventMessage, string exChange, string queue, string replyto = "", string corelationid = "", string contenttype = "")
        {
            ...
            try
            {
                ...
                using (SendConnection)
                {
                    ...
                    using (SendChannel)
                    {
                        var messageSerializer = MessageSerializerFactory.CreateMessageJsonSerializerInstance(this.EncodingName); //反序列化消息
        ...
        private void consumer_Received(object sender, BasicDeliverEventArgs e)
        {
            ...
            try
            {
                result = EventMessage.BuildEventMessageJSONResult(e.Body, this.EncodingName, e.BasicProperties.CorrelationId, e.BasicProperties.ReplyTo, e.BasicProperties.ContentType); //获取消息返回对象
        ...
        public string RpcClient(EventMessage eventMessage, string exChange, string queue, string replyqueue)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        var result = EventMessage.BuildEventMessageJSONResult(res.Body, this.EncodingName);
                        SendChannel.BasicAck(res.DeliveryTag, false);
                        return MessageSerializerFactory.CreateMessageJsonSerializerInstance(this.EncodingName)
